...
def _normalize_heatmap_probability(probability_map: np.ndarray, focus_mask: np.ndarray) -> np.ndarray:
    ...
    focus_strength = cv2.GaussianBlur(focus_mask.astype(np.float32), (0, 0), sigmaX=4.2)
    ...
    if focus_values.size > 0:
        ...
        if high - low >= 1e-6:
            ...
            normalized = np.zeros_like(probability_map, dtype=np.float32)
    else:
        normalized = np.zeros_like(probability_map, dtype=np.float32)

    # Ensure the heatmap visibly follows the tobacco mask so the heat view and mask stay consistent.
    normalized = np.maximum(normalized, focus_strength * 0.96).astype(np.float32)
    normalized = np.where(focus_strength > 0.03, normalized, normalized * 0.04).astype(np.float32)
    return normalized
